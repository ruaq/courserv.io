        ...
        <div class="-mx-4 mt-8 overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:-mx-6 md:mx-0 md:rounded-lg">
            <table class="min-w-full divide-y divide-gray-300">
                ...
                <tr>
                    ...
                    <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">{{ _i('team') }}</th>
                    <th scope="col" class="hidden px-3 py-3.5 text-left text-sm font-semibold text-gray-900 sm:table-cell">{{ _i('Description') }}</th>
                    <th scope="col" class="px-3 py-3.5 text-right text-sm font-semibold text-gray-900">{{ _i('leading') }}</th>
                ...
                @foreach($positions as $position)
                    <tr>
                        ...
                        <td class="py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                            @can('update', $position)
                                <x-button.link wire:click="edit({{ $position->id }})"><i class="fa-solid fa-pen-to-square"></i></x-button.link>
                            @else
    ...
    <form wire:submit.prevent="save">
        <x-modal.dialog wire:model.defer="showEditModal">
            ...
            <x-slot name="content">
                <div class="space-y-3">
                    <div>
                        <label for="name" class="block text-sm font-medium text-gray-700">{{ _i('position title') }}</label>
                        <div class="relative mt-1 rounded-md shadow-sm">
                            <input type="text" wire:model.lazy="editing.title" name="{{ _i('team name') }}" id="name" class="@error('editing.title') border-red-300 text-red-900 placeholder-red-300 focus:border-red-500 focus:outline-none focus:ring-red-500 @enderror block w-full rounded-md border-gray-300 pr-10 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="{{ _i('team name') }}" @error('editing.title') aria-invalid="true" aria-describedby="title-error" @enderror>
                            @error('editing.title')
                    ...
                    <div x-data="{ leadingPosition: @entangle('editing.leading').defer }" class="space-y-3">
                        <div class="flex items-center">
                            <button type="button" class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent bg-gray-200 transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2" role="switch" aria-checked="false" x-ref="switch" :aria-checked="leadingPosition" @click="leadingPosition = !leadingPosition" x-state:on="Enabled" x-state:off="Not Enabled" :class="{ 'bg-indigo-600': leadingPosition, 'bg-gray-200': !(leadingPosition) }">
                                <span class="sr-only">{{ _i('leading position') }}</span>
                                <span class="pointer-events-none relative inline-block h-5 w-5 translate-x-0 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out" x-state:on="Enabled" x-state:off="Not Enabled" :class="{ 'translate-x-5': leadingPosition, 'translate-x-0': !(leadingPosition) }">
                                        <span class="absolute inset-0 flex h-full w-full items-center justify-center opacity-100 transition-opacity duration-200 ease-in" aria-hidden="true" x-state:on="Enabled" x-state:off="Not Enabled" :class="{ 'opacity-0 ease-out duration-100': leadingPosition, 'opacity-100 ease-in duration-200': !(leadingPosition) }">
                                        ...
                                        <span class="absolute inset-0 flex h-full w-full items-center justify-center opacity-0 transition-opacity duration-100 ease-out" aria-hidden="true" x-state:on="Enabled" x-state:off="Not Enabled" :class="{ 'opacity-100 ease-in duration-200': leadingPosition, 'opacity-0 ease-out duration-100': !(leadingPosition) }">
                            ...
                            <span class="ml-3" id="public-bookable-label" @click="leadingPosition = !leadingPosition; $refs.switch.focus()">
                    ...
                    <div>
                        ...
                        <select id="team" wire:model.lazy="editing.team_id" name="team" required class="mt-1 block w-full rounded-md border-gray-300 py-2 pl-3 pr-10 text-base focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm">
